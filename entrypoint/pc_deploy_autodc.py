...
def create_vm(cluster, vm_name, image_name, network_name, assigned_ip):
  ...
  """
    ...
    cluster.execute(
        "acli vm.disk_create {vm_name}"
        " clone_from_image={image_name}".format(vm_name=vm_name, image_name=image_name)
    )
    cluster.execute(
        "acli vm.nic_create {vm_name}"
        " network={network} ip={ip}".format(
            vm_name=vm_name, network=network_name, ip=assigned_ip
        )
    )
