...
from helpers.calm import (file_to_dict, body_via_v3_get,
                          body_via_v3_post, update_via_v3_put)
  ...
  try:
    ...
    for item in mp_items["entities"]:
      ...
      if item["status"]["app_state"] == "PENDING":
        ...
        body = body_via_v3_get(pc_external_ip, "calm_marketplace_items",
                               pc_password, item["metadata"]["uuid"]).json
        del body["status"]
        body["spec"]["resources"]["app_state"] = "ACCEPTED"

        # Update the item
        resp = update_via_v3_put(pc_external_ip, "calm_marketplace_items",
                                 pc_password, item["metadata"]["uuid"],
                                 body)
        ...
        if (resp.code == 200 or resp.code == 202):
          ...
          raise Exception(f"{item['status']['name']} bp approved" +
                          f" failed with:\n" +
